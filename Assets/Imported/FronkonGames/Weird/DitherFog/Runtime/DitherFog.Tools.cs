...
namespace FronkonGames.Weird.DitherFog
{
  ...
  public sealed partial class DitherFog
  {
    ...
    private static readonly DitherFog[] NoEffects = new DitherFog[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static DitherFog Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is DitherFog)
              return scriptableRendererData.rendererFeatures[i] as DitherFog;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static DitherFog[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is DitherFog)
                  effects.Add(feature as DitherFog);
          }
