...
namespace FronkonGames.Weird.Crystal
{
  ...
  public sealed partial class Crystal
  {
    ...
    private static readonly Crystal[] NoEffects = new Crystal[0];
    ...
    public static Crystal Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Crystal)
              return scriptableRendererData.rendererFeatures[i] as Crystal;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Crystal[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Crystal)
                  effects.Add(feature as Crystal);
          }
