...
namespace FronkonGames.Weird.Kaleidoscope
{
  ...
  public sealed partial class Kaleidoscope
  {
    ...
    private static readonly Kaleidoscope[] NoEffects = new Kaleidoscope[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Kaleidoscope Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Kaleidoscope)
              return scriptableRendererData.rendererFeatures[i] as Kaleidoscope;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Kaleidoscope[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Kaleidoscope)
                  effects.Add(feature as Kaleidoscope);
          }
