...
namespace FronkonGames.Weird.Kaleidoscope.Editor
{
  ...
  public abstract partial class Inspector : VolumeComponentEditor
  {
    ...
    /// <summary> Draws an AnimationCurve field with reset. </summary>
    protected void DrawCurveWithReset(string name, string label = null, AnimationCurve defaultValue = null)
    {
      SerializedDataParameter parameter = UnpackParameter(name);
      if (parameter == null)
        return;
      ...
      EditorGUILayout.BeginHorizontal();
      {
        ...
        EditorGUILayout.BeginHorizontal();
        {
          AnimationCurve value = parameter.value.animationCurveValue;
          if (value == null)
            value = new AnimationCurve();

          EditorGUI.BeginChangeCheck();
          value = EditorGUILayout.CurveField(displayLabel, value);
          if (EditorGUI.EndChangeCheck())
            parameter.value.animationCurveValue = value;
          ...
          bool canReset = defaultValue != null && !CurveEquals(value, defaultValue);
          if (ResetButton(defaultValue, canReset) == true && defaultValue != null)
            parameter.value.animationCurveValue = new AnimationCurve(defaultValue.keys);
        }
        EditorGUILayout.EndHorizontal();
    ...
    private bool CurveEquals(AnimationCurve a, AnimationCurve b)
    {
      if (a == null || b == null)
        return a == b;
