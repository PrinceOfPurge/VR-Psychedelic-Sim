...
namespace FronkonGames.Weird.FireTunnel
{
  ...
  public sealed partial class FireTunnel
  {
    ...
    private static readonly FireTunnel[] NoEffects = new FireTunnel[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static FireTunnel Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is FireTunnel)
              return scriptableRendererData.rendererFeatures[i] as FireTunnel;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static FireTunnel[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is FireTunnel)
                  effects.Add(feature as FireTunnel);
          }
