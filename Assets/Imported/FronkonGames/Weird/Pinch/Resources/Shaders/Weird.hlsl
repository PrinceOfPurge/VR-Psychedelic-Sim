...
inline float SampleDepthLOD(float2 uv)
{
...
inline float SampleLinear01DepthLOD(float2 uv)
{
  return Linear01Depth(SampleDepthLOD(uv), _ZBufferParams);
}
...
inline float3 pow3(float3 x, float y) { return pow(max(x, 0.0), (float3)y); }
...
struct WeirdAttributes
{
  uint vertexID : SV_VertexID;
  UNITY_VERTEX_INPUT_INSTANCE_ID
};

struct WeirdVaryings
{
...
WeirdVaryings WeirdVert(WeirdAttributes input)
{
  WeirdVaryings output;
...
{
  const half separatorWidth = 2.0 * TEXEL_SIZE.x;
