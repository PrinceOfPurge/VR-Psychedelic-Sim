...
namespace FronkonGames.Weird.Bubbles
{
  ...
  public sealed partial class Bubbles
  {
    ...
    private static readonly Bubbles[] NoEffects = new Bubbles[0];
    ...
    public static Bubbles Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Bubbles)
              return scriptableRendererData.rendererFeatures[i] as Bubbles;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Bubbles[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Bubbles)
                  effects.Add(feature as Bubbles);
          }
