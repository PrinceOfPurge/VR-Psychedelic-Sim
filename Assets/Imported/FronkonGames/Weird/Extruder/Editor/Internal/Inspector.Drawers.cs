...
namespace FronkonGames.Weird.Extruder.Editor
{
  ...
  public abstract partial class Inspector : VolumeComponentEditor
  {
    protected void DrawFloatSliderWithReset(string name, string label = null)
    {
      ...
      EditorGUILayout.BeginHorizontal();
      {
        ...
        EditorGUILayout.BeginHorizontal();
        {
          ...
          if (ResetButton(value != attr.value) == true)
            value = attr.value;
    ...
    /// <summary> Draws a MinMaxSlider with reset for volume parameters. </summary>
    protected void DrawMinMaxSliderWithReset(string minName, string maxName, string label, float minLimit, float maxLimit, float defaultMinLimit = 0.0f, float defaultMaxLimit = 1.0f)
    {
      ...
      GUIContent displayLabel = new(label, $"[{minValue:0.00} .. {maxValue:0.00}]");
      ...
      EditorGUILayout.BeginHorizontal();
      {
        ...
        EditorGUILayout.MinMaxSlider(displayLabel, ref minValue, ref maxValue, minLimit, maxLimit);

        EditorGUI.EndDisabledGroup();
        int oldIndentLevel = EditorGUI.indentLevel;
        EditorGUI.indentLevel = 0;
        EditorGUILayout.PropertyField(minParam.overrideState, GUIContent.none, GUILayout.Width(16));
        EditorGUI.indentLevel = oldIndentLevel;
        EditorGUI.BeginDisabledGroup(!isOverridden);

        if (ResetButton(minValue != defaultMinLimit || maxValue != defaultMaxLimit) == true)
        {
