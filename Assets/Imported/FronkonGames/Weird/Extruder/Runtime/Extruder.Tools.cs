...
namespace FronkonGames.Weird.Extruder
{
  ...
  public sealed partial class Extruder
  {
    ...
    private static readonly Extruder[] NoEffects = new Extruder[0];
    ...
    public static Extruder Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Extruder)
              return scriptableRendererData.rendererFeatures[i] as Extruder;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static Extruder[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Extruder)
                  effects.Add(feature as Extruder);
          }
